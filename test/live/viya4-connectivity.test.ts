...
describe("live: Viya 4 connectivity", function () {
  ...
  it("authenticates as the configured user", async function () {
    ...
    const response = await fetch(
      new URL("/identities/users/@currentUser", target.baseUrl),
      {
        headers: {
          Accept: "application/vnd.sas.identity+json",
    ...
    assert.equal(
      response.status,
      200,
      `expected 200 from /identities/users/@currentUser, got ${String(response.status)}`,
    );
