...
export class RunOutputChannel implements vscode.Disposable {
   ...
   * channel reads exactly like the program's own stdout. A rich output this
   * text channel does not render inline (an image, an HTML table) gets one
   * localised line pointing at the Result panel, which does show it (3d-ii);
   * see `./render`.
   */
  writeOutput(output: RichOutput): void {
    for (const line of renderRichOutput(output)) {
      ...
      this.channel.appendLine(
        line.mime === "image/png"
          ? vscode.l10n.t("[an image was produced — see the Result panel]")
          : vscode.l10n.t(
