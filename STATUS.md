...
again). **6d-i (favourites) is code-complete 2026-09-10** — `ContentAdapter.addToFavorites`
/ `removeFromFavorites` (a `reference` member `POST` / a `DELETE` of that record —
finding 6.13, which also settles Finding 80: `memberCount` is a phantom, the
members listing is the only authority), a `markFavorites` opt on `getChildItems`
that stamps `isInMyFavorites` + `favoriteUri`, a `.fav` `contextValue` suffix +
`favoriteAction` on the presentation, two flat commands, and the `=~` migration
of the content menu `when` clauses. `npm run verify` green (1506 unit; coverage
95.38 lines / 95.29 branches / 95.07 functions / 95.38 statements), 321
integration passing, `npm run check:docs` green. **Adversarial pass done before
the PR (no blocking findings); [PR #157](https://github.com/Shai-Alit/sas-py-vscode/pull/157)
open.** Review findings folded in over two rounds, all local, one push each:
- **Round 1 (Codex, 2 × Major, one root cause):** a Recycle Bin child kept a bare
  `sasContent:folder`/`:file` `contextValue`, so the new add-favourite command
  showed on it → a `.recycled` `contextValue` suffix (parallel to `.fav`; every
  content `when` regex is `$`-anchored, so a `.recycled` item matches none — this
  also tightens the pre-existing 6c-i menu on bin content) + an `inRecycleBin`
  early-out in `favorite()`. Plus `npm run docs:reference` (the `docs` CI job's
  `docs:reference:check` isn't in `npm run verify`) and a merge of `origin/main`
  for the CI-workflow fix (#156).
- **Round 2:** **(blocking)** `favoritesFolder()` memoised the per-account My
  Favorites `addMember` href, but a `ContentAdapter` is cached per *endpoint* and
  reused across profile switches → one account's favourite could land in
  another's; memoisation dropped, `@myFavorites` re-fetched every call.
  **(likely blocking)** `typeNameOf`/`isContainer` only special-cased
  `type: "child"`, so a favourite browsed *inside* My Favorites (wire
  `type: "reference"`) read as a non-navigable leaf — folders wouldn't expand,
  files wouldn't open; `typeNameOf` now defers to `contentType` for `"reference"`
  too. **(minor)** a direct integration test for the `favorite()` bin early-out.
**6d-ii (recycle bin) is next** — recycle/restore via the move primitive
(findings 6.14/6.15), empty-bin, the read-only `sasContentReadOnly` scheme.
