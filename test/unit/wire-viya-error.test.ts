...
import {
  ...
  describeViyaError,
  readViyaError,
} from "../../src/wire/viyaError";

/**
 * The one claim worth testing here is that {@link readViyaError} cannot throw.
 * It runs on the failure path, frequently on the failure path of a teardown,
 * and a parser that throws there turns a diagnosable problem into an opaque one
 * — so it is fed a deliberately hostile range of bodies below. {@link
 * describeViyaError}'s job is small by comparison: prefer the human sentence,
 * fall back to the generic message, and never render an empty pair of brackets.
 *
 * Split out of `compute-problems.test.ts` in Phase 6a-i, when the envelope
 * reader moved to `src/wire/` (ADR-0025) so the Folders and Files services
 * could share it.
 */
