...
of the content menu `when` clauses. `npm run verify` green (1503 unit; coverage
95.37 lines / 95.31 branches / 95.06 functions / 95.37 statements), 320
integration passing. **Adversarial pass done before the PR (no blocking
findings); [PR #157](https://github.com/Shai-Alit/sas-py-vscode/pull/157) open.**
The Codex PR review then raised two Major findings, one root cause — a Recycle
Bin child kept a bare `sasContent:folder`/`:file` `contextValue`, so the new
add-favourite command showed on it. Fixed by a `.recycled` `contextValue` suffix
(parallel to `.fav`; every content `when` regex is `$`-anchored so a `.recycled`
item matches none of them — this also tightens the pre-existing 6c-i menu on bin
content) plus an `inRecycleBin` early-out in the `favorite()` handler. Also
picked up: the CI `docs` job needed `npm run docs:reference` re-run for the two
new commands, and `origin/main`'s CI-workflow fix (#156) was merged in.
...
- **6c — mutations (create/rename/move/delete).** Split into three sub-slices
  ...
  **6c-iii** `getParent`/`TreeView.reveal` + the finding-101 `ancestors` probe.
  ...
  - **6d (favourites + recycle bin)** is split into 6d-i / 6d-ii. **6d-i
    ...
    verify` green (1503 unit; coverage 95.37/95.31/95.06/95.37), 320 integration.
