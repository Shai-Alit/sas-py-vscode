...
| `pythonOnViya.userProvidedCertificates` | `array` | `[]` | `machine` | Absolute paths to PEM certificate files to trust when connecting to SAS Viya, for a deployment behind a private certificate authority or one that serves an incomplete chain. Each file is added to a dedicated HTTPS agent used only for this extension's requests, so it does not change what any other extension trusts. Read once when the window loads; a change takes effect on the next reload. A path that cannot be read is reported in the Python on Viya log and the others are still used. Machine-scoped so that a checked-in workspace settings file cannot widen TLS trust. |
