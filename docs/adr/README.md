...
| [0038](0038-every-run-is-wrapped-in-a-named-ods-destination.md) | Every run is wrapped in a named ODS HTML5 destination so `SAS.show()` output is shown; its body file has a fixed name and is captured apart from ADR-0019's diff | Accepted — amended 2026-09-25 by ADR-0039: two syntax-check recovery lines now precede the wrapper |
| [0039](0039-every-job-switches-syntax-check-mode-off.md) | Every job switches SAS syntax-check mode off (and undoes a SAS-set `OBS=0`) before anything else runs, so one failed SAS step cannot poison the session | Accepted |
