...
import { type CasClient, type CasClientConfig } from "../../src/cas/client";
import {
  CasSession,
  type AccountLike,
  type SessionLike,
} from "../../src/cas/casSession";

/**
 * The adapter lifecycle behind the CAS view — build, endpoint-cache,
 * sign-out clear — and the silent, account-hinted token flow. Mirrors
 * `content-session.test.ts` exactly: `CasSession` is `ContentSession`'s twin
 * (ADR-0033), keyed on endpoint for the identical reason.
 */
...
function harness(
  ...
  const configs: CasClientConfig[] = [];
  const noopClient: CasClient = {
