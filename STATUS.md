...
in `phase-11.md`, not blocking). **The Phase 6→7/8 between-phase housekeeping
(`HOUSEKEEPING.md`) — which 6e was the last thing holding open — is otherwise
done** (stale refs, ADR-0030, and manual-test-pass.md §15/16 were handled at
the checkpoint itself, `0655977`); **one open item from that checkpoint's own
testing inventory remains outstanding and needs Sean's call**: the
top-level-folder permanent-delete confirmation (`manual-test-pass.md`'s
unchecked row, `phase-6.md` line ~941) has still never been live-exercised —
either run it now to close housekeeping cleanly, or explicitly defer it with
a stated reason per `HOUSEKEEPING.md` item 6. **6a is done** (split 6a-i + 6a-ii) and
**6b is done**; **6c** is
