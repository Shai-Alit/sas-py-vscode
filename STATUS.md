...
assumption on the second, now tracked as its own 8a punch-list item. See
`phase-8.md`'s Runbook and Probe findings (8.7/8.8/8.9) for the full
account. **8b (authenticated CAS session helper) is next.**
...
| 8 — CAS and SWAT | **8a (CAS browsing) done 2026-09-11** — a read-only CAS tree (servers/caslibs/tables/columns), no session of any kind needed ([ADR-0033](docs/adr/0033-cas-adapter-shape.md)); a pre-PR manual-test crash (duplicate/dropped tables from `casManagement`'s own unstable pagination order) found and fixed 2026-09-12, `sortBy=name` seeded on every paginated request (Finding 8.9); `npm run verify` green (1669 unit; coverage 95.82/95.46/95.66/95.82), 382 integration. 8b next. | `docs/phases/phase-8.md` |
