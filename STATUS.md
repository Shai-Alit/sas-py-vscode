...
- **7c-ii (table properties/columns static viewer) is code-complete
  ...
  boolean vs. `AbortSignal.aborted` style note). One real but pre-existing gap
  — a stuck "Loading…" panel if `openTable`/`getColumns` *rejects* rather than
  resolving `{ok:false}` — is shared with `DataViewerPanelManager`'s own
  identical `loadTable` and deliberately left open rather than fixed
  piecemeal in only one of the two files; a small follow-up covering both is
  the right shape for it, not this slice.
  **[PR #158](https://github.com/Shai-Alit/sas-py-vscode/pull/158) opened
  ...
  nonce test). Full account in `phase-7.md`'s 7c-ii Runbook entry.
