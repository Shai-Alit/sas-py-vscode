...
begins with `=`, `+`, `-`, `@`, a tab, a carriage return or a line feed,
which a spreadsheet application that opens the file can treat as a formula — turn on
`pythonOnViya.csvExport.guardFormulaInjection` to have such a cell prefixed
with a leading `'` instead (never applied to a numeric column, so a negative
value is unaffected), or open an export of a table you do not trust in a text
editor or `pandas.read_csv`, not directly in a spreadsheet.
...
- **Your own CASUSER library is not in the tree.** Expected today — only
  ...
  release.
- **An export or the data viewer stops with "SAS Viya's answer was larger
  than this extension reads at once (limit 1 MB)".** A page of a very wide
  table exceeded the 1 MiB response-size cap this extension enforces. The
  connection is fine. Nothing is written to your destination; export a
  narrower table or view instead.
