...
done; PR not yet opened.** Finding 6.12 pinned the `/folders/ancestors` wire
shape (superseding finding 101). This clone also had `npm install` run to
reconcile `node_modules` with Sean's concurrent Phase 7b merge (React +
ag-grid) that `main` fast-forwarded onto. **6d (favourites + recycle bin) is
next.**
...
- **6c — mutations (create/rename/move/delete).** Split into three sub-slices
  ...
  **6c-iii** `getParent`/`TreeView.reveal` + the finding-101 `ancestors` probe.
  ...
  - **6c-iii — `getParent` / `TreeView.reveal`.** Code-complete 2026-09-10;
    adversarial pass done, PR not yet opened. `ContentAdapter.getParentOfItem`
