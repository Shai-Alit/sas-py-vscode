...
on (`phase-9.md`'s 9a Runbook entry has the detail). Manual test items added
— `docs/dev/manual-tests/phase-9.md` 9.1–9.5 — not yet run. **Committed
locally on `feat/9a-notebook-controller`; not yet pushed, no PR opened.**
...
| 8 — CAS and SWAT | **8a (CAS browsing) done — [PR #169](https://github.com/Shai-Alit/sas-py-vscode/pull/169) merged 2026-09-13, squash `610d3f7`** — a read-only CAS tree (servers/caslibs/tables/columns), no session of any kind needed ([ADR-0033](docs/adr/0033-cas-adapter-shape.md)); a pre-PR manual-test crash (duplicate/dropped tables from `casManagement`'s own unstable pagination order) found and fixed 2026-09-12, `sortBy=name` seeded on every paginated request (Finding 8.9), live-retest-confirmed fixed 2026-09-13; `npm run verify` green (1669 unit; coverage 95.82/95.46/95.66/95.82), 382 integration. Loaded-vs-unloaded CAS table icon parity flagged 2026-09-13, folded into 8b. **8b (authenticated CAS session helper) code-complete 2026-09-13** — the non-negotiable job-log check (8.14) passed; one manual-test bug (8.18, snippet inserting into non-`.py` files) found and fixed same day, live-reconfirmed 2026-09-13 (Sean). PR review (Codex + Claude reviewer) repeatedly flagged the command's network chain as lacking a `withProgress`/cancellation path (an earlier "no timeout" framing of the same finding was checked and found incorrect — `CasClient`/`ComputeClient` already default-timeout every call) — fixed by wrapping the chain in a cancellable progress notification with a shared `AbortSignal`; `npm run verify` green (1689 unit; coverage 95.87/95.45/95.72/95.87), 402 integration. **[PR #170](https://github.com/Shai-Alit/sas-py-vscode/pull/170) merged 2026-09-13, squash `671db19`.** **8c (CAS tables in the data viewer) code-complete 2026-09-13** — Findings 8.11–8.13 (a CAS table's row data lives at the end of a `casManagement` → Data Tables API → `rowSets` relation chain; sort/filter travel as plain query parameters, no server-side view needed); `DataViewerPanelManager`/`OpenTablePanel` generalized behind a new `TableSource` interface (`src/data/tableSource.ts`) so the same panel serves both `LibraryTableSource` (`src/data/librarySource.ts`, carrying 7c's own view-creation logic unchanged) and the new `CasTableSource` (`src/cas/casTableSource.ts`); `CasAdapter` gained `openTable`/`getRows`; `pythonOnViya.openCasTable` opens a CAS table node in the shared panel. `npm run coverage` green (1675 unit; coverage 95.91/95.46/95.75/95.91), 403 integration passing, `check:docs` green. Adversarial self-review run before the PR opens, 2026-09-13 — no blocking findings. **[PR #171](https://github.com/Shai-Alit/sas-py-vscode/pull/171) opened 2026-09-13.** Review found one blocking issue — `CasTableSource.key` omitted the profile/endpoint, so switching deployments could reveal a panel still bound to a previous deployment's `CasAdapter` — fixed by giving `CasAdapter` its own `endpoint` field and folding it into the key, mirroring `LibraryTableSource`'s `${profileId}\n...` shape, with a new regression test; the same review's non-blocking observation that the `TableSource` generalization lacked an ADR is now [ADR-0034](docs/adr/0034-table-source-abstraction.md). `npm run coverage` green (1675 unit; coverage 95.92/95.46/95.75/95.92), 404 integration passing, `check:docs`/`check:copyright`/`check:secrets` clean. A second Codex review round then found one Major (the dispose path's bare `void this.source.close()` dropped the old `.then()`-logged cleanup-failure guarantee — restored with a `.catch` backstop, plus a regression test) and one Minor (`openCasTable` missing from `package.json`'s `commandPalette` exclusion list, unlike its Library-table sibling) — both fixed same-branch, one push; `npm run coverage` green (1703 unit; coverage unchanged), 412 integration passing. **Merged 2026-09-14, squash `bb80b92`. 8c is done.** | `docs/phases/phase-8.md` |
| 9 — Notebooks | **9a (spike + controller registration) code-complete 2026-09-14 — adversarial review done, no blocking findings; committed locally, not yet pushed, no PR.** No `ms-toolsai.jupyter` dependency (confirmed live, ADR-0024 unchanged); `src/notebook/notebookController.ts` registers a `NotebookController` against `jupyter-notebook` with a placeholder `executeHandler` — real execution is 9b. `npm run coverage`/`test:integration` green; manual test items 9.1–9.5 added, not yet run. | `docs/phases/phase-9.md` |
