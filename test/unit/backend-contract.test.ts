...
import { createFakeBackend, fakeProgram } from "../helpers/fake-backend";

/**
 * ADR-0015, clause by clause.
 *
 * `src/backend/backend.ts` is types, and types compile whether or not anyone can
 * satisfy them. Nothing implements the seam until slice 3a, so without this file
 * the interface would sit for two slices with no evidence that its clauses are
 * consistent with each other — that a serial backend can reject cleanly, that
 * cancellation composes with streaming, that a degraded backend has somewhere to
 * put its refusal.
 *
 * The subject is therefore the *contract*, not the double. Every test below
 * should read as a sentence from the ADR, and when slice 3a's `PROC PYTHON`
 * backend arrives it should be able to run this same file.
 */
...
describe("the ExecutionBackend contract", () => {
  describe("connecting", () => {
    ...
    it("does not create a session of its own", () => {
      ...
      // absence except that the interface offers no way to do it — no `create`,
      // no session argument on `connect` — so this test exists to fail loudly in
      // review if one is ever added.
      const backend = createFakeBackend();
      assert.ok(
  ...
  describe("running one program at a time", () => {
    ...
    it("rejects a second run and does not queue it", async () => {
      ...
      // visible policy decision and belongs to the slice with a status bar in
      // it, so the evidence here is that *nothing was accepted*.
      const backend = createFakeBackend();
      await backend.connect();
  ...
  describe("the program is bytes", () => {
    it("carries the bytes through unchanged", async () => {
      // ADR-0014 in executable form. The hostile string is the one from probe
      // finding 31 — the line that ends a `SUBMIT` block from inside a Python
      // string — and the point is that it is not special here, because there is
      // no code text for anything to interpolate it into.
      const source = "s = '''\nendsubmit;\n'''\nprint(len(s))\n";
      const backend = createFakeBackend();
      await backend.connect();
  ...
  describe("a fresh namespace", () => {
    it("is refused explicitly by a backend that cannot provide one", async () => {
      // The clause that keeps a degraded backend honest. Quietly reusing the
      // namespace would present as the user's own bug, in their own code, and
      // nothing in the log would say otherwise.
      const backend = createFakeBackend({ freshNamespace: false });
      await backend.connect();
      const result = await backend.execute(fakeProgram(), {
        freshNamespace: true,
      });
  ...
  describe("transfer", () => {
    it("reports that nothing ran when the bytes never arrived", async () => {
      // The price ADR-0015 pays for a single `execute` rather than a `stage`
      // then a `run`: this distinction has to live in the failure value. If the
      // member disappears, so does a caller's ability to know a retry is safe.
      const backend = createFakeBackend({
        transferProblem: {
  ...
  describe("the handle streams", () => {
    it("yields output before the run has settled", async () => {
      // The reason the seam returns a handle instead of a finished result. If
      // this ever has to wait for `done`, slices 2c, 3b and 3d lose the thing
      // they were built on.
      const backend = createFakeBackend();
      await backend.connect();
      const accepted = await backend.execute(fakeProgram(), {
    ...
    it("lets a caller await the outcome without iterating at all", async () => {
      // A backend that stalls waiting for a consumer would make `done` a trap
      // for every caller that only wants to know whether it worked.
      const backend = createFakeBackend();
      await backend.connect();
      const accepted = await backend.execute(fakeProgram(), {
        freshNamespace: false,
    ...
    it("settles once, and says the same thing every time it is awaited", async () => {
      ...
      // A second ending must not overwrite the first: whichever conclusion the
      // run reached is the one the user saw.
  ...
  describe("cancelling", () => {
    ...
    it("succeeds and does nothing for a run that already finished", async () => {
      // A user who hits Cancel as the run completes has not made a mistake, and
      // must not be shown a failure for winning a race.
      const backend = createFakeBackend();
      await backend.connect();
      const accepted = await backend.execute(fakeProgram(), {
        freshNamespace: false,
  ...
  describe("capabilities", () => {
    it("answers without I/O, and admits it has not probed the runtime", async () => {
      ...
      // version before you can run Python (§2.3). `unprobed` is that separation
      // made visible, and slice 3e is what widens it.
      const backend = createFakeBackend();
